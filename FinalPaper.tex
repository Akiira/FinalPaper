...
The following section has been broken down into the subsections for each major aspect of the proposed algorithm. These will be presented generally in the temporal order in which they are used by the main simulation loop below. First the main simulation loop is introduced to give context throughout the rest of the sections. Next the notion of input parameter ranges and the simulation's range set will be introduced. The method used to initialize the population with test suites will be discussed, followed by a discussion of how fitness is calculated and associated with organisms. The main tools used for test case generation will be defined, including test case crossover, test case mutation, and local optimization. Finally, the algorithm's perturbation operations, range set adaptation, test suite crossover, and test suite mutation will be defined.
