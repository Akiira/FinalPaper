...
After several generations of no improvement, the range set is adapted based on the usefulness of its ranges. First, the range set deletes any ranges below one standard deviation usefulness. All other ranges are split into two equal sized ranges. The two ranges above and below the original range are also added to the range set; these new adjacent ranges have the same size as the original range. Finally, a new random range is added to the range set to avoid missing ranges not found during initialization or a previous range adaptation.
